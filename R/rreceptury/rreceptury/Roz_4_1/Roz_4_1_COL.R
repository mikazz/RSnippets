...
for(i in 1:(length(METHODS))) {
  ...
  plot(1:length(error$train), error$train, "l", col = "blue",
       lwd = 2, ylim = range(unlist(error)),
       xlab = "Liczba zmiennych", ylab = "MSE",
       main = paste("Regresja", METHODS[i]))
  lines(1:length(error$valid), error$valid, col = "red", lwd = 2)
...
plot(LAMBDAS, ridge.errors$train, "l", log = "x", col = "blue",
     ylim = range(unlist(ridge.errors)),
     ylab = "MSE", main = "Regresja grzbietowa", lwd = 2)
lines(LAMBDAS, ridge.errors$valid, col = "red", lwd = 2)
points(LAMBDAS[which.min(ridge.errors$valid)],
       min(ridge.errors$valid), col = "red", pch = 19, cex = 2)
...
plot(BOUNDS, lasso.errors$train, type = "l", col = "blue", lwd = 2,
     ylim = range(unlist(lasso.errors)),
     log = "x", ylab = "MSE", main = "Regresja LASSO")
lines(BOUNDS, lasso.errors$valid,col = "red", lwd = 2)
