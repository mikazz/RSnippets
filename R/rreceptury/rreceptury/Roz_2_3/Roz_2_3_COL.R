...
DoHistogram <- function(values, values.name) {
	...
	lines(density.estimate, col = "red", lwd = 2)
}

DoScatterplot <- function(x.values, x.name) {
	plot(x.values, DATA_SET$MORT, xlab = x.name, ylab = "MORT", main = NULL)
	lines(smooth.spline(x.values, DATA_SET$MORT), col = "red", lwd = 2)
	abline(lm(DATA_SET$MORT ~ x.values)$coef, col = "blue", lwd = 2, lty = 2)
}
...
legend("left", c("dane", "liniowy", "wyg³adzany"), bty = "n",
       col = c("black", "blue", "red"), pch = c(1, -1, -1),
