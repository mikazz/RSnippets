...
for (n in 1:length(NEURONS)) {
  plot(X, y, xlim = c(-2, 2), ylim = c(-5, 6), main = main[n], col = gray(.7))
	lines(X, TRUE_Y, lwd = 4)
	lines(X, bootstrap.average[[n]], lwd = 3, col = gray(0.4), lty = 2)
  lines(X, bootstrap.ci95[[n]][1,], lwd = 1, col = gray(0.4))
  lines(X, bootstrap.ci95[[n]][2,], lwd = 1, col = gray(0.4))
  lines(X, single.model.prediction[[n]], col = gray(0.6), lwd = 3, lty = 3)
}
legend("bottomright", bty = "o",
       leg = c("Teoretyczna", "Pojedyncza sieæ", "Bagging", "95%-CI"),
       lwd = c(3, 3, 3, 1), col = c("black", gray(c(0.6, 0.4, 0.4))),
       lty = c(1, 3, 2, 1), y.inter = 0.25, x.inter = 0.25)
